...
docker run --rm --entrypoint bash cli-tool-docker:local-test -lc 'cat /etc/os-release; cha --version || true; navi --version; zoxide --version; yq --version; tenv version; command -v terraform; command -v tofu; command -v terragrunt; command -v atmos; gcloud --version; pipx --version; dig -v; 7z | sed -n "2p"'
```
